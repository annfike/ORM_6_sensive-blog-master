...
def index(request):
    #posts = Post.objects.annotate(Count('likes', distinct=True), count_comments=Count('comments', distinct=True)).prefetch_related('author')
    #popular_posts = posts.order_by('-likes__count')
    #most_popular_posts = popular_posts[:5]  

    #fresh_posts = Post.objects.annotate(count_comments=Count('comments')).order_by('published_at').prefetch_related('author')
